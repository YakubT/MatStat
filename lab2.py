...
def findParametrsSquare(poligonFunc):
    res = []
    matrKoef = []
    sumXIPow = [len(poligonFunc),0,0,0,0]
    n = len(poligonFunc)
    for i in range(n):
        for j in range(1,5):
    ...
    sumYiX = [0,0,0]
    for i in range(n):
        sumYiX[0]+=poligonFunc[i][1]*(poligonFunc[i][0]**2)
        sumYiX[1]+=poligonFunc[i][1]*poligonFunc[i][0]
        sumYiX[2]+=poligonFunc[i][1]
    matrKoef.append([sumXIPow[4],sumXIPow[3],sumXIPow[2]])
    matrKoef.append([sumXIPow[3],sumXIPow[2],sumXIPow[1]])
    matrKoef.append([sumXIPow[2],sumXIPow[1],sumXIPow[0]])
    A = numpy.array(matrKoef)
    b = numpy.array(sumYiX)
    return numpy.linalg.solve(A,b)


def funcRozp(x,a,b,c):
    return 1/3.0*(a*x**3)+((b/2)*x**2)+c*x

def PirsCriteria(tabl, end,a,b,c):
    table = []
    ends = []
    ...
    for i in range(m):
        pi = round((funcRozp(ends[i][1], a,b,c)-funcRozp(ends[i][0],a,b,c))/(funcRozp(ends[m-1][1], a,b,c)-funcRozp(ends[0][0],a,b,c))*100)/100
        piSum = piSum + pi
        print(str(i+1).center(2),"  ",round(ends[i][0]),'-',ends[i][1],"     ",str(table[i][1]).center(5),str(pi).center(3),"  ",pi*n,"  ",round(100*(table[i][1]-pi*n)**2)/100, "  ",round(100*(table[i][1]-pi*n)**2/(n*pi))/100)
...
def solve(x):
    ...
    print("Підберу параметри параболічної функціональної залежності y=ax^2+bx+c")
    poligonFunc = []
    for i in range(m):
    ...
    print("Нульова Гіпотеза Ho - генеральна сукупність має параболічний закон розподілу з параметрами а,b,c, що вказані вище")
    print("Перевірю гіпотезу за критерієм Пірсона з рівнем значущості 0,05")
    PirsCriteria(table,ends,params[0],params[1],params[2])
