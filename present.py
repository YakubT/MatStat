...
def drawPoligonAndTeorRozp2(table,n,params):
    turtle.Screen().bgpic('nopic')
    k=100
    drawPoligon(table,n)
    x = []
    ...
    for i in range(k):
        ...
        y.append(params[0]*(x[i]**2)+params[1]*x[i]+params[2])
...
def buildRegression():
    turtle.Screen().bgpic('nopic')
    turtle.setheading(0)
    turtle.Screen().title("Лінія регресії: Залежність співвідношення євро до долара від відношення показників інфляції у %")
    turtle.clear()
    turtle.up()
    xstart = 44
    xend = 350
    ...
    step = (xend-xstart)/kP
    x = [xstart, xend]
    y = [round(119.31-0.068*xstart),round(119.31-0.068*xend)]
    drawCoordSystem('x','y')
    turtle.setpos(start[0]-50,start[1]-100)
    plotFunc(x,y,'red',2,7)
    turtle.up()
...
def drawModa2(maximIndex,table,ends,y,n, maxim):
    turtle.Screen().bgpic('nopic')
    rMod = start[0]-70+0.75*width*(ends[maximIndex][1]-ends[0][0])/(ends[len(table)-1][1]-ends[0][0])
    modR = (rMod,start[1]+y[maximIndex]*height/maxim)
    modR1 = (ends[maximIndex][1],y[maximIndex])
    if (maximIndex!=0):
        ...
        beforeMode = (start[0]-70+0.75*width*(ends[maximIndex-1][1]-ends[0][0])/(ends[len(table)-1][1]-ends[0][0]),
         ...
         beforeMode1 = (ends[0][0],0)
         beforeMode = (start[0]-70+0.75*width*(ends[0][0]-ends[0][0])/(ends[len(table)-1][1]-ends[0][0]),
                  start[1])
    drawDashedLine(modR,beforeMode,3)
    modL = (start[0]-70+0.75*width*(ends[maximIndex][0]-ends[0][0])/(ends[len(table)-1][1]-ends[0][0]),start[1]+y[maximIndex]*height/maxim)
    modL1  = (ends[maximIndex][0],y[maximIndex])
    if (maximIndex!=len(table)-1):
        afterMode = (start[0]-70+0.75*width*(ends[maximIndex+1][0]-ends[0][0])/(ends[len(table)-1][1]-ends[0][0]),
                  start[1]+y[maximIndex+1]*height/maxim)
        afterMode1 = (ends[maximIndex+1][0],y[maximIndex+1])
    else:
        afterMode = (start[0]-70+0.75*width*(ends[maximIndex][1]-ends[0][0])/(ends[len(table)-1][1]-ends[0][0]),
                  start[1])
        afterMode1 = (ends[maximIndex][1],0)
    drawDashedLine(modL,afterMode,3)
    k1 = (beforeMode1[1]-modR1[1])/(beforeMode1[0]-modR1[0])
    b1 = modR1[1]-modR1[0]*(beforeMode1[1]-modR1[1])/(beforeMode1[0]-modR1[0])
    k2 = (afterMode1[1]-modL1[1])/(afterMode1[0]-modL1[0])
...
def drawHistograma2(table,ends,n):
    turtle.Screen().bgpic('nopic')
    turtle.Screen().title("Гістограма")
    turtle.clear()
    turtle.up()
    ...
    for i in range(len(table)):
        if (table[i][1]>table[maximIndex][1]):
            maximIndex = i
    drawModa2(maximIndex,table,ends,y,n,maxim)
    drawModa2(len(table)-1,table,ends,y,n,maxim)
    turtle.setpos(start[0]-30,start[1]-100)
    turtle.write("Гістограма",font=('arial',20,'bold'))
    turtle.setheading(0)
...
def funcRozp2(x,params):
    return 1/3*(params[0]*x**3)+((params[1]/2)*x**2)+(params[2]*x);


def integralSer2(x,params):
    return 1/4*(params[0]*x**4)+((params[1]/3)*x**3)+((params[2]/2)*x**2);

def serTeor2(l,r,params):
    return (integralSer(r,params)-integralSer(l,params))/(funcRozp(r,params)-funcRozp(l,params))

def PirsCriteria2(tabl, end,params):
    table = []
    ends = []
    for i in range(len(tabl)):
        table.append(tabl[i])
    ...
    while i < (len(table)-1):
        if (table[i][1]<5):
            table[i+1] =  ((table[i+1][0]*(ends[i+1][1]-ends[i+1][0])+table[i][0]*(ends[i][1]-ends[i][0]))/(ends[i+1][1]-ends[i][0]),table[i+1][1] +table[i][1])
            ends[i+1] = (ends[i][0],ends[i+1][1])
            table.pop(i)
            ends.pop(i)
        else :
            i = i+1
    if (table[len(table)-1][1]<5):
        table[len(table)-2] = ((table[len(table)-2][0]*(ends[len(table)-2][1]-ends[len(table)-2][0])+table[len(table)-1][0]*(ends[len(table)-1][1]-ends[len(table)-1][0]))/
                               (ends[len(table)-1][1]-ends[len(table)-2][0]),
        table[len(table)-2][1] + table[len(table)-1][1])
        ends[len(table)-2] = (ends[len(table)-2][0],ends[len(table)-1][1])
        table.pop(len(table)-1)
        ends.pop(len(table)-1)
...
def TextRes1():
    turtle.Screen().title("Узагальнені текстові результати дослідження. Аркуш 1")
    turtle.clear()
    turtle.setpos(0,0)
    turtle.Screen().bgpic('pres_1.gif')
    
def TextRes2():
    turtle.Screen().title("Узагальнені текстові результати дослідження. Аркуш 2")
    turtle.clear()
    turtle.setpos(0,0)
    turtle.Screen().bgpic('pres_2.gif')
       
def solve2(x):
    n = len(x)
    print("Якубишин КП-12 Співідношення показників інфляції у ЄС та США у відсотках")
    print("Згенерована вибірка")
    print(x,"\n")
    x.sort()
    print("Ранжована вибірка:")
    ...
    print("Незміщенна точкова оцінка дисперсії", sNezm)
    print()
    print("Підберу параметри квадратичної функціональної залежності y=ax^2+bx+c")
    poligonFunc = []
    for i in range(m):
        poligonFunc.append((table[i][0],table[i][1]/n))
    params = [6*10**(-6), -0.0042, 0.6732]
    print("a = ",params[0])
    print("b = ",params[1])
    print("c = ", params[2])
    print()
    print("y =",params[0],"* x^2",params[1],"* x","+",params[2])
    serKvPoh = math.sqrt(sNezm/n)
    dov = 0.95
    t = 1.96 #при довірчий ймовірності 0,95
    delta = t*serKvPoh
    ...
    print("Довірчий інтервал для генеральної дисперсії:",lD,"< disp <", rD)
    print()
    print("Нульова Гіпотеза Ho - генеральна сукупність має поліміальний закон розподілу з параметрами а,b,c що вказані вище, альтернативна - не узгоджується")
    print("Перевірю гіпотезу за критерієм Пірсона з рівнем значущості 0,05")
    PirsCriteria2(table,ends,params)
    print()
    a0 = 99
    print("Висуну Гіпотезу Ho - що генеральне середнє, дорівнює", a0,", H1 - a < ",a0," Лівостороння критична область")
    znach = 0.05
    ...
    print()
    print("Лабораторна 3")
    print("Рівняння регресії X по Y:")
    print("x = 119,31-0,068y")
    print("вибірковий коефіцієнт кореляції = ",-0.673144)
    print("tstat = ",9.01)
    print("tkr = ",1.984)
    print("-0,7473 <= p <= -0,55")
    print("-0,07 <= betaxy <= -0,06595")
    print("Введіть у вікні 5 - для побудови полігона частот, 6 - для побудови гістограми,7 -для побудови кумуляти, 8 - теоретичний розподіл, 9 - лінія рергресії")
    print("Введіть у вікні a - для виводу узагальних текстових результатів дослідження")
    turtle.Screen().listen()
    turtle.Screen().onkey(lambda: drawPoligon(table,n),'5')
    turtle.Screen().onkey(lambda: drawHistograma2(table,ends,n),'6')
    turtle.Screen().onkey(lambda: drawCumuliata2(table,chastNakArr,n),'7')
    turtle.Screen().onkey(lambda: drawPoligonAndTeorRozp2(table,n,params),'8')
    turtle.Screen().onkey(lambda: buildRegression(),'9')
    turtle.Screen().onkey(lambda: TextRes1(),'a')
    turtle.Screen().onkey(lambda: TextRes2(),'b') 
    turtle.Screen().mainloop()
...
n = 100
f = open("dataY.txt", "r")
xTMP = f.readline().split()
x2 = []
for i in range(33):
    x2.append(float(xTMP[i]))
    x2.append(float(xTMP[i]))
    x2.append(float(xTMP[i]))
    if (i==0):
            x2.append(float(xTMP[i]))
