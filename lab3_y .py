...
def drawModa(maximIndex,table,ends,y,n, maxim):
...
def drawHistograma(table,ends,n):
    ...
    for i in range(len(table)):
        if (table[i][1]>table[maximIndex][1]):
            maximIndex = i
    drawModa(maximIndex,table,ends,y,n,maxim)
    drawModa(len(table)-1,table,ends,y,n,maxim)
...
def solve(x):
    ...
    turtle.Screen().onkey(lambda: buildRegression(),'5')
